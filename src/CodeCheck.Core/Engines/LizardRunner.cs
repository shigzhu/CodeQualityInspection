...
public sealed class LizardRunner : IAnalysisEngine
{
    ...
    {
        return ParseIssues(output, files, DefaultRuleMappingResolver.Value, complexityThreshold, functionLinesThreshold);
    }

    public static List<Issue> ParseIssues(string output, IReadOnlyList<ScanInputFile> files, RuleMappingResolver mappingResolver, int complexityThreshold = 10, int functionLinesThreshold = 100)
    {
        ...
        foreach (var line in output.Split(new[] { '\r', '\n' }, StringSplitOptions.RemoveEmptyEntries))
        {
            ...
            {
                var mapping = mappingResolver.Resolve("lizard", "cyclomatic-complexity", language);
                issues.Add(new Issue { IssueId = $"LIZARD-{issueIndex:D6}", RuleId = mapping.RuleId, EngineRuleId = mapping.EngineRuleId, Severity = "Warning", Language = language, Engine = "lizard", Message = $"Function cyclomatic complexity is {ccn}, greater than {complexityThreshold}.", File = file?.RelativePath ?? location, Line = lineNumber });
                issueIndex++;
            }

            if (length > functionLinesThreshold)
            {
                var mapping = mappingResolver.Resolve("lizard", "function-length", language);
                issues.Add(new Issue { IssueId = $"LIZARD-{issueIndex:D6}", RuleId = mapping.RuleId, EngineRuleId = mapping.EngineRuleId, Severity = "Warning", Language = language, Engine = "lizard", Message = $"Function length is {length}, greater than {functionLinesThreshold}.", File = file?.RelativePath ?? location, Line = lineNumber });
                issueIndex++;
            }
        }
